...
from core.eval import quiescient_search
...
def evaluate_moves_multi_sort(board, moves, alpha, beta, depth, stop_time):
    gen_all_masks()
    ...
    for move in all_moves:
        ...
        if bSearchPv:
            score, _, mc = pvsuche_multi_sort(board_copy, -beta, -alpha, depth - 1, stop_time)
            score = -score
            move_count += mc
        else:
            score, _, mc = pvsuche_multi_sort(board_copy, -alpha - 1, -alpha, depth - 1, stop_time)
            score = -score
            move_count += mc
            if score > alpha:
                score, _, mc = pvsuche_multi_sort(board_copy, -beta, -alpha, depth - 1, stop_time)
...
def pvs_multi_sort_qs(board, time_limit):
    stop_time = time.time() + time_limit
    glob_best_move = None
    depth = 1
    while time.time() < stop_time:
        t1 = time.time()
        _, best_move, move_count = pvsuche_first_multi_sort(board, float('-inf'), float('inf'), depth, stop_time)
        t2 = time.time()
...
def pvsuche_first_multi_sort(board, alpha, beta, depth, stop_time):
    best_move = None
    unsort_moves = generate(board)
    all_moves = sort_moves(unsort_moves)
    ...
    with multiprocessing.Pool() as pool:
        results = pool.starmap(evaluate_moves_multi_sort, [(board, moves, -beta, -alpha, depth, stop_time) for moves in move_chunks])
...
def pvsuche_multi_sort(board, alpha, beta, depth, stop_time):
    move_count = 0
    best_move = None
    if depth == 0 or game_ending(board) or stop_time < time.time():
        if stop_time < time.time():
            return 0, best_move, 1
        return quiescient_search(board, alpha, beta, 0), best_move, 1
...
if __name__ == "__main__":
    ...
    print(board1)
    print("------------------------")
    print("PVS", pvs_multi_sort(board1, 1, time_limit))
    board2 = Board(fen2)
    print("PVS", pvs_multi_sort(board2, 1, time_limit))