...
def pvs_sort_qs(board,  time_limit):
    stop_time = time.time() + time_limit
    glob_best_move = None
    depth = 1
    while time.time() < stop_time:
        t1 = time.time()
        _, best_move, move_count = pvsuche_sort(board, float('-inf'), float('inf'), depth, stop_time)
        t2 = time.time()
...
def pvsuche_sort(board, alpha, beta, depth, stop_time):
    best_move = None
    move_count = 0
    if depth == 0 or game_ending(board) or stop_time < time.time():
        if stop_time < time.time():
            return 0, best_move, 1
        return quiescient_search(board, alpha, beta, 0), best_move, 1  # You need to implement the 'quiesce' function separately

    bSearchPv = True
    moves = generate(board)
    all_moves = sort_moves(moves)
    ...
    for move in all_moves:  # You need to define the 'all_moves' list or generator
        ...
        if bSearchPv:
            score, _, mc = pvsuche_sort(board_copy, -beta, -alpha, depth - 1, stop_time)
            score = -score
            move_count += mc
        else:
            score, _, mc = pvsuche_sort(board_copy, -alpha - 1, -alpha, depth - 1, stop_time)
            score = -score
            move_count += mc
            if score > alpha:  # In fail-soft, 'score < beta' is common, but not included here
                score, _, mc = pvsuche_sort(board_copy, -beta, -alpha, depth - 1, stop_time)  # Re-search
                score = -score
                move_count += mc
...
if __name__ == "__main__":
    ...
    print("PVS", pvs_sort(board1, 1, time_limit))
    board2 = Board(fen2)
    print(len(generate(board2)))
    print("PVS", pvs_sort(board2, 1, time_limit))
